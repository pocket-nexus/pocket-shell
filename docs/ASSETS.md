...
| Desktop `assets/icons/render.py`, `pixel-icons.py` and 36 PNG bakes | Original Blender scenes and pixel-grid recipes for Files, Devices, game consoles and media players across Aqua, XP and Classic 98, with shipping icons at 16/32/64 px. Cycles models, materials, cameras and lights are recreated by the recipe; normal builds copy these offline bakes into the ignored icon directory at 1×/2× density. Editable `.blend` files and full-size renders stay in `.pocket-build/validation/blender-icons/`. |
